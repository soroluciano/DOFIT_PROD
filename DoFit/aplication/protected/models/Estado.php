...
class Estado extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('id_estado, estado, fhcreacion, cusuario', 'required'),
			array('id_estado', 'numerical', 'integerOnly'=>true),
			array('estado, cusuario', 'length', 'max'=>60),
			array('fhultmod', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id_estado, estado, fhcreacion, fhultmod, cusuario', 'safe', 'on'=>'search'),
		);
	}
